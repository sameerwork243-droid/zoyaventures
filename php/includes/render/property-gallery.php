...
// Renders the original gallery structure: mobile static image (d-xl-none),
// desktop main image + 2 sub-images (d-none d-xl-flex), and the amenity-slider
// (Photos / Location / Share buttons with slick arrow buttons).
...
function property_gallery(array $srcs, string $type, ?string $location = null, ?string $title = null): string
{
    ...
    // Main image (left, spans both rows on desktop).
    $html .= '<div class="pe-gallery-main img-zoom">';
    if ($main) {
        $html .= '<img loading="eager" data-gallery-open="0" src="' . esc(cft($main, 1200, 675)) . '" alt="' . esc($type) . ' - main image">';
    }
    if ($n > 1) {
    ...
    // Right rail: up to 2 sub-images stacked to match the main image height.
    if (count($side)) {
    ...
    // Lightbox (rendered closed; opened via property.js from data-srcs).
